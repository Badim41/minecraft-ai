...
function formatPlan(targetItem, { required, steps, leftovers }) {
    ...
    if (Object.keys(required).length > 0) {
        // TRANSLATED_RU
        lines.push('Вам не хватает следующих предметов:');
        ...
        // TRANSLATED_RU
        lines.push('У вас есть все предметы, необходимые для создания этого предмета!');
        // TRANSLATED_RU
        lines.push('Вот ваш план крафта:');
    }
    ...
    if (Object.keys(required).some(item => item.includes('oak')) && !targetItem.includes('oak')) {
        // TRANSLATED_RU
        lines.push('Примечание: Для этого рецепта можно использовать любой вид древесины.');
    }

    if (Object.keys(leftovers).length > 0) {
        // TRANSLATED_RU
        lines.push('\nУ вас останутся лишние предметы:');
