...
export class SelfPrompter {
    ...
    async startLoop() {
        ...
        while (!this.interrupt) {
            // TRANSLATED_RU
            const msg = `Вы выполняете автопромптинг с целью: '${this.prompt}'. Ваш следующий ответ ДОЛЖЕН содержать команду с синтаксисом: !commandName. Ответьте:`;
            ...
            if (!used_command) {
                ...
                if (no_command_count >= MAX_NO_COMMAND) {
                    // TRANSLATED_RU
                    let out = `Агент не использовал команду в последних ${MAX_NO_COMMAND} автопромптах. Автопромптинг остановлен.`;
